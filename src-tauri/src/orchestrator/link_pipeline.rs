...
Respond with valid JSON: {"queries": ["query1", "query2", ...]}. No markdown, no explanation."#;
        ...
        let parsed: QueriesResponse = serde_json::from_str(&response.content)
            .map_err(|e| {
                ...
                llm.report_invalid_response("link_search_planner", &message, &response);
                message
            })?;

        if parsed.queries.is_empty() {
            llm.report_invalid_response("link_search_planner", "LLM returned an empty query list", &response);
