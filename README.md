...
**Thinking / reasoning effort** controls the model's reasoning before its answer. **Auto** preserves the safe defaults: structured Ollama requests disable thinking (GPT-OSS uses Low), while other providers keep their defaults. **Provider default** leaves reasoning to the server. **Off**, **On**, **Low**, **Medium**, **High**, and **Max** request an explicit mode or level; support depends on the model and provider. GPT-OSS on Ollama accepts Low/Medium/High and cannot use Off or Max. Changes apply to new runs after Save.

**Max output tokens** is a per-request allowance, potentially shared by thinking and answer tokens. It is separate from the run's cost/time limits and the provider account quota. Higher reasoning effort can leave fewer tokens for the answer.

Model request issues show the provider, model, stage, requested output cap, any reported input/output/thinking token counts, and whether a retry was scheduled. Output limits, context limits, provider quotas, rate limits, authentication, and network failures have distinct explanations and corrective actions. Issues remain visible when individual pages are skipped and are available in Run History. Other errors show a short explanation, an action to take, and collapsible technical details.
