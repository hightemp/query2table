...
describe('LLM provider settings', () => {
	...
	it('keeps reasoning changes local until Save and restores the saved choice', async () => {
		...
		expect(screen.getByText(/Auto disables thinking/)).toHaveTextContent(
			'Provider default leaves thinking unchanged'
		);
		await fireEvent.click(screen.getByRole('button', { name: 'Save' }));
		await waitFor(() => expect(get(settings).get('llm_reasoning_effort')).toBe('high'));
		page.unmount();
		render(SettingsPage);
		expect(screen.getByRole('combobox', { name: /^Thinking \/ reasoning effort / })).toHaveValue(
			'high'
		);
	});
	...
	it('explains unsupported GPT-OSS effort choices only for Ollama providers', async () => {
		...
		await fireEvent.input(screen.getByLabelText(/^Ollama Model /), {
	...
	it('shows save failures and keeps unsaved edits available for retry', async () => {
		...
		await fireEvent.change(
			screen.getByRole('combobox', { name: /^Thinking \/ reasoning effort / }),
			{ target: { value: 'medium' } }
		);
	...
	it('selects an OpenRouter model from the filtered catalog and saves its ID', async () => {
		...
		expect(
			within(screen.getByRole('listbox', { name: 'OpenRouter models' })).getAllByRole('option')
		).toHaveLength(1);
		...
		page.unmount();
		render(SettingsPage);
		expect(screen.getByRole('combobox', { name: /^OpenRouter Model / })).toHaveValue(
			'openai/gpt-test'
		);
	});
	...
	it('switches provider fields and preserves edits when saving and reopening', async () => {
		...
		await fireEvent.input(screen.getByLabelText(/^Ollama Cloud Model /), {
		...
		await fireEvent.input(screen.getByLabelText(/^API Base URL /), {
			target: { value: 'http://localhost:8080/v1' },
		});
		await fireEvent.input(screen.getByLabelText(/^Model Required /), {
			target: { value: 'llama-model' },
		});
...
describe('settings save transactions', () => {
	...
	it('keeps an edit made during Save dirty and saves its latest value on retry', async () => {
		render(SettingsPage);
		const model = screen.getByRole('combobox', { name: /^Thinking \/ reasoning effort / });
		await fireEvent.change(model, { target: { value: 'low' } });
		let finish!: () => void;
		const original = settings.save;
		vi.spyOn(settings, 'save').mockImplementationOnce(async (key, value) => {
		...
		await fireEvent.click(screen.getByRole('button', { name: 'Save' }));
		await fireEvent.change(model, { target: { value: 'high' } });
		finish();
		await waitFor(() => expect(screen.getByRole('button', { name: 'Save' })).toBeEnabled());
		expect(model).toHaveValue('high');
		expect(get(settings).get('llm_reasoning_effort')).toBe('low');
		await fireEvent.click(screen.getByRole('button', { name: 'Save' }));
		await waitFor(() => expect(get(settings).get('llm_reasoning_effort')).toBe('high'));
	});
	it('keeps failed values dirty after a partial save and discards to the last saved state', async () => {
		render(SettingsPage);
		const effort = screen.getByRole('combobox', { name: /^Thinking \/ reasoning effort / });
		await fireEvent.change(effort, { target: { value: 'low' } });
		await fireEvent.input(screen.getByLabelText(/^Max output tokens /), {
			target: { value: '8192' },
		});
		const original = settings.save;
		const save = vi
